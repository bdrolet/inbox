...
**Danger:** the `email-events` Pub/Sub topic and its publisher IAM binding (`google_pubsub_topic.email_events` in `terraform/pubsub.tf`, `google_pubsub_topic_iam_member.process_cf_email_events_publisher` in `terraform/iam.tf`) were introduced in this branch's *first* commit (7f6b73d) and were already applied to GCP well before the code deploy/merge. A naive `git revert` of the full inbox PR merge reverts **that commit too**, deleting both resources from the Terraform state on the next apply. `.github/workflows/deploy.yml` runs `terraform apply -auto-approve` automatically on any push to `main` that touches `terraform/**` — so a plain revert-then-push would auto-destroy the live `email-events` topic the tasks service depends on, taking down the tasks pipeline instead of just the direct-Asana fallback.

If the tasks side can't be fixed quickly, roll back like this instead:

1. `git revert` the inbox PR merge commit (do **not** push yet). This restores the direct Asana path (deleted files, `ASANA_*` env, and the `process_cf_asana` IAM binding all come back) but also stages the deletion of the `email_events` topic + `process_cf_email_events_publisher` binding, since those live in the same merge.
2. **Before pushing**, re-add the two email-events resources the revert just removed — either cherry-pick commit 7f6b73d's two hunks back on top of the revert (`git show 7f6b73d -- terraform/pubsub.tf terraform/iam.tf` to get them, since `git checkout 7f6b73d -- terraform/pubsub.tf terraform/iam.tf` won't work cleanly — those files also carry the later Asana-removal edits), or manually re-add the `google_pubsub_topic.email_events` resource in `terraform/pubsub.tf` and the `google_pubsub_topic_iam_member.process_cf_email_events_publisher` resource in `terraform/iam.tf`.
3. Confirm with `terraform plan` (via `/terraform-plan`) that the topic and publisher binding show **no changes** and only the Asana-path resources reappear as adds, then push and let `deploy.yml` apply.

Pub/Sub retries (`RETRY_POLICY_RETRY`, at-least-once) plus the tasks side's `external.gid` dedupe make the direct-Asana/event overlap safe during the window — drained events become no-op skips, not duplicates. Never roll back by destroying tasks infrastructure (topic/DB/secrets carry state). Full procedure: "Rollback procedures" in `/Users/ben/src/tasks/docs/superpowers/plans/2026-07-15-tasks-repo-setup.md`.
