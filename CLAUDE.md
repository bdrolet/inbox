...
**People ownership (shipped):** inbox owns mail only. The separate `people` repo (github.com/bdrolet/people) owns everyone Ben corresponds with — Google Contacts (source of truth), a derived `people` index, a HubSpot mirror capped at 1000 most-recent contacts, and `people-api`. Inbox publishes `email_classified` (unchanged) and a new `email_sent` event from a second Graph subscription on Sent Items (`clientState` `inbox-webhook-sent`, secret `graph-sent-subscription-id`); the webhook stamps a `folder` Pub/Sub attribute and `main.py::process` routes `sentitems` to `handlers/sent.py`, which publishes and stores nothing. At classify time the pipeline fetches sender context from `people-api` (`clients/people_api.py`, 2 s timeout, fail-open; authenticated with a Google ID token minted for the `inbox-process-cf` SA by `clients/gcp_auth.py` — people-api is behind Cloud Run IAM, no shared secret). The `senders` table and HubSpot client are gone. Do not add contact or HubSpot logic here again. See `docs/superpowers/specs/2026-09-03-people-service-extraction-design.md`.
...
| **API** | Cloud Run service `inbox-api` (FastAPI) — email search + outbound email; behind Cloud Run IAM |
| **Redirector** | Cloud Run service `inbox-redirect` — `GET /r/{uuid}` only, public by design (ntfy taps carry no credential); same image as inbox-api, entrypoint `api.redirect_app:app`; SA `inbox-redirect@` with DB + MSAL + Azure app secrets only (`terraform/redirect.tf`) |
...
| `msal-token-cache` | Processor CF + renew CF + `inbox-api` — MSAL refresh token. **Versions are runtime-owned, not Terraform-seeded**: every writer adds a version on refresh, then **destroys** all but the newest `MSAL_CACHE_KEEP_VERSIONS` (default 3) enabled versions |
